    ...
    {                                                               \
        ...
        if (argc > 1)                                               \
            littleEndian = EJSVAL_TO_BOOLEAN(args[1]);              \
                                                                    \
        char* data = (char*)_ejs_dataview_get_data (EJSVAL_TO_OBJECT(_this)); \
        elementtype val;                                            \
...
#define EJS_TYPED_ARRAY(EnumType, ArrayType, arraytype, elementtype, elementSizeInBytes) \
 ...
 {                                                                      \
     ...
     _ejs_specop_set(_this, args[0], args[1], _this);                     \
     return args[1];                                                    \
 }                                                                      \
                                                                        \
 static ejsval                                                          \
...
_ejs_arraybuffer_get_data (EJSObject* buf)
{
    ...
    if (array_buffer->dependent) {
        return (char*)_ejs_arraybuffer_get_data (EJSVAL_TO_OBJECT(array_buffer->data.dependent.buf)) + array_buffer->data.dependent.offset;
    }
...
_ejs_typedarray_get_data(EJSObject* arr)
{
    ...
    return (char*)buffer_data + typed_array->byteOffset;
}
...
_ejs_dataview_get_data(EJSObject* view)
{
    ...
    return (char*)buffer_data + data_view->byteOffset;
}
...
_ejs_arraybuffer_specop_get (ejsval obj, ejsval propertyName, ejsval receiver)
{
    ...
    // we also handle the length getter here
    if (EJSVAL_IS_STRING(propertyName) && !EJSChar_strcmp (_ejs_buffer_byteLength, EJSVAL_TO_FLAT_STRING(propertyName))) {
...
#define ADD_TYPEDARRAY_SPECOPS(ArrayType, arraytype)                   \
    EJSSpecOps _ejs_##arraytype##array_specops = {                     \
        #ArrayType "Array",                                            \
        (SpecOpGetPrototypeOf)OP_INHERIT, /* [[GetPrototypeOf]] */                           \
        (SpecOpSetPrototypeOf)OP_INHERIT, /* [[SetPrototypeOf]] */                           \
        (SpecOpIsExtensible)OP_INHERIT, /* [[IsExtensible]] */                             \
        (SpecOpPreventExtensions)OP_INHERIT, /* [[PreventExtensions]] */                        \
        (SpecOpGetOwnProperty)_ejs_##arraytype##array_specop_get_own_property,               \
        (SpecOpDefineOwnProperty)_ejs_##arraytype##array_specop_define_own_property,            \
        (SpecOpHasProperty)_ejs_##arraytype##array_specop_has_property,                   \
        (SpecOpGet)_ejs_##arraytype##array_specop_get,                            \
        (SpecOpSet)_ejs_##arraytype##array_specop_set,                            \
        (SpecOpDelete)OP_INHERIT, /* [[Delete]] */                                   \
        (SpecOpEnumerate)OP_INHERIT, /* [[Enumerate]] */                                \
        (SpecOpOwnPropertyKeys)OP_INHERIT, /* [[OwnPropertyKeys]] */                          \
        (SpecOpAllocate)_ejs_typedarray_specop_allocate,                               \
        (SpecOpFinalize)_ejs_typedarray_specop_finalize,                               \
        (SpecOpScan)_ejs_typedarray_specop_scan                                    \
    };
