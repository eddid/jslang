...
_ejs_Number_prototype_toFixed (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 10. Else x < 10^21
        ...
        //     a. Let n be an integer for which the exact mathematical value of n / 10^f â€?x is as close to zero as 
        ...
        if (f != 0) {
            ...
            //        iii. Let a be the first kâ€“f elements of m, and let b be the remaining f elements of m. 
...
_ejs_Number_prototype_toPrecision (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 11. If x = 0, then 
    if (x == 0) {
        //     a. Let m be the String consisting of p occurrences of the code unit 0x0030 (the Unicode character â€?â€?. 
        m = zeros[p];
    ...
    // 12. Else x != 0, 
        ...
        //     a. Let e and n be integers such that 10^(pâ€?) <= n < 10^p and for which the exact mathematical value of n * 10^(eâ€“p+1) â€?x is as close to zero as possible.
        //        If there are two such sets of e and n, pick the e and n for which n * 10^(eâ€“p+1) is larger. 
        //     b. Let m be the String consisting of the digits of the decimal representation of n (in order, with no leading zeroes). 
        //     c. If e < â€? or e >= p, then 
        //        i. Assert: e != 0 
        //        ii. Let a be the first element of m, and let b be the remaining pâ€? elements of m. 
    ...
    // 13. If e = pâ€?, then return the concatenation of the Strings s and m. 
    if (e == p - 1)
        return _ejs_string_concat(s, m);
    // 14. If e >= 0, then 
    if (e >= 0) {
        //     a. Let m be the concatenation of the first e+1 elements of m, the code unit 0x002E (Unicode character â€?â€?, and the remaining pâ€?(e+1) elements of m. 
    }
    // 15. Else e < 0, 
    else {
        //     a. Let m be the concatenation of the String "0.", â€?e+1) occurrences of code unit 0x0030 (the Unicode character â€?â€?, and the String m. 
        int z = -(e+1);
        ejsval zstr;
        if (z > 0 && z <= 21)
            zstr = zeros[z];
...
_ejs_Number_isSafeInteger (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 5. If abs(integer) â‰?253-1, then return true.
...
#define OBJ_METHOD(x) EJS_INSTALL_ATOM_FUNCTION(_ejs_Number, x, _ejs_Number_##x)
    ...
    _ejs_object_setprop (_ejs_Number, _ejs_atom_EPSILON, _ejs_nan);
    ...
    _ejs_object_setprop (_ejs_Number, _ejs_atom_MAX_VALUE, jsMax);
    _ejs_object_setprop (_ejs_Number, _ejs_atom_MIN_VALUE, jsMin);
    _ejs_object_setprop (_ejs_Number, _ejs_atom_NaN, _ejs_nan);
    _ejs_object_setprop (_ejs_Number, _ejs_atom_NEGATIVE_INFINITY, jsNegativeInfinity);
    _ejs_object_setprop (_ejs_Number, _ejs_atom_POSITIVE_INFINITY, jsPositiveInfinity);
