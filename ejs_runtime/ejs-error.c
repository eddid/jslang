...
_ejs_Error_prototype_toString (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    ejsval name = _ejs_specop_get(_this, _ejs_atom_name, _this);
    if (EJSVAL_IS_NULL_OR_UNDEFINED(name))
        name = _ejs_atom_Error;

    ejsval message = _ejs_specop_get(_this, _ejs_atom_message, _this);
...
{
    ejsval toString = _ejs_function_new_without_proto (_ejs_null, _ejs_atom_toString, (EJSClosureFunc)_ejs_Error_prototype_toString);
...
_ejs_throw_nativeerror (EJSNativeErrorType error_type, ejsval message)
{
    ...
    char *message_utf8 = _ejs_string_to_utf8(EJSVAL_TO_STRING(message));
    _ejs_log ("throwing exception with message %s\n", message_utf8);
    free (message_utf8);
