...
{
    ReactionTaskArg *arg = (ReactionTaskArg *)malloc(sizeof(ReactionTaskArg));
...
{
    ResolveThenableTaskArg *arg = (ResolveThenableTaskArg *)malloc(sizeof(ResolveThenableTaskArg));
...
_ejs_Promise_create (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    if (!EJSVAL_IS_UNDEFINED(F)) {
        ...
        proto = _ejs_specop_get(F, _ejs_atom_prototype, F);
    }
