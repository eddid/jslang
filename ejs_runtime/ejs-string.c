...
_ejs_String_prototype_substr (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    /* 6. Compute min(max(Result(3),0), Result(4)â€“Result(5)). */
...
_ejs_String_prototype_split (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    /* 5. If limit is undefined, let lim = 2^32â€?; else let lim = ToUint32(limit). */
...
codepoint_to_codeunits(int64_t codepoint, jschar* units)
{
    ...
    // 2. If cp â‰?65535, then return cp.
    if (codepoint <= 65535) {
        *units = (jschar)codepoint;
        return 1;
    }
    
    // 3. Let cu1 be floor((cp â€?65536) / 1024) + 55296. NOTE 55296 is 0xD800. 
    *units = (jschar)(floor((double)(codepoint - 0x10000) / 1024) + 0xD800);
    // 4. Let cu2 be ((cp â€?65536) modulo 1024) + 56320. NOTE 56320 is 0xDC00.
...
_ejs_String_raw(ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 11. If literalSegments â‰?0, then return the empty string.
...
_ejs_String_prototype_codePointAt(ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 12. Return ((first â€?0xD800) Ã— 1024) + (second â€?0xDC00) + 0x10000. 
...
_ejs_String_prototype_repeat(ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 7. If n is +âˆ? then throw a RangeError exception.
    if (isinf(n_))
...
_ejs_string_iterator_new (ejsval string)
{
    ...
    /* 4. Set iteratorâ€™s [[IteratedString]] internal slot to s. */
    iterator->iterated = s;

    /* 5. Set iteratorâ€™s [[StringIteratorNextIndex]] internal slot to 0. */
