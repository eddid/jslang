...
_ejs_Array_prototype_shift (ejsval env, ejsval _this, uint32_t argc, ejsval*args)
{
    ...
    // 8. Call the [[Delete]] internal method of O with arguments ToString(lenâ€?) and true.
    _ejs_specop_delete(O, NUMBER_TO_EJSVAL(len-1), EJS_TRUE);

    // 9. Call the [[Put]] internal method of O with arguments "length", (lenâ€?) , and true.
...
_ejs_Array_prototype_unshift (ejsval env, ejsval _this, uint32_t argc, ejsval*args)
{
    ...
    // 6. Repeat, while k > 0,
    while (k > 0) {
        //   a. Let from be ToString(kâ€?).
        ejsval from = ToString(NUMBER_TO_EJSVAL(k-1));

        //   b. Let to be ToString(k+argCount â€?).
...
_ejs_Array_prototype_concat (ejsval env, ejsval _this, uint32_t argc, ejsval* args)
{
    ...
    if (EJSVAL_IS_ARRAY(O)) {
        ...
        //       i. Let thisRealm be the running execution contextâ€™s Realm.
        //       ii. If thisRealm and the value of Câ€™s [[Realm]] internal slot are the same value, then
...
_ejs_Array_prototype_lastIndexOf (ejsval env, ejsval _this, uint32_t argc, ejsval* args)
{
    ...
    /* 6. If n â‰?0, then let k be min(n, len â€?1). */
    if (n >= 0)
        k = MIN(n, len-1);
    /* 7. Else, n < 0 */
    else
...
{
    // start, deleteCount [ , item1 [ , item2 [ , â€?] ] ]
    ...
    if (itemCount < actualDeleteCount) {
        ...
        k = actualStart;
        /*     b. Repeat, while k < (len â€?actualDeleteCount) */
        ...
        while (k > (len - actualDeleteCount + itemCount)) {
            /*        i. Call the [[Delete]] internal method of O with arguments ToString(kâ€?) and true. */
    ...
    /* 13. Else if itemCount > actualDeleteCount, then */
        ...
        /*     a. Let k be (len â€?actualDeleteCount). */
...
_ejs_array_iterator_new(ejsval array, EJSArrayIteratorKind kind)
{
    ...
    /* 4. Set iteratorâ€™s [[IteratedObject]] internal slot to O. */
    iter->iterated = O;

    /* 5. Set iteratorâ€™s [[ArrayIteratorNextIndex]] internal slot to 0. */
    iter->next_index = 0;

    /* 6. Set iteratorâ€™s [[ArrayIterationKind]] internal slot to kind. */
    iter->kind = kind;
