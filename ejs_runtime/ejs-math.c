...
{
    double current_max = jsNegativeInfinity.asDouble;
...
{
    double current_min = jsPositiveInfinity.asDouble;
    for (int i = 0; i < argc; i ++) {
        double d = ToDouble(args[i]);
        if (isnan (d))
            return _ejs_nan;
...
_ejs_Math_imul (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 4. ReturnIfAbrupt(b).
    // 5. Let product be (a Ã— b) modulo 2^32.
    uint64_t product = ((uint64_t)a * (uint64_t)b) % (2LL<<32);

    // 6. If product â‰?2^31, return product âˆ?2^32, otherwise return product.
    if (product >= 2LL<<31)
...
_ejs_Math_sign (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 2. If x is âˆ?, the result is âˆ?.
    // 3. If x is +0, the result is +0.
    // 4. If x is negative and not âˆ?, the result is âˆ?.
...
_ejs_Math_log10 (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 3. If x is +0, the result is +0.
    // 4. If x is âˆ?, the result is âˆ?.
    if (x_ == 0)
        return x;

    // 5. If x is 1, the result is +0.
    // 6. If x is +âˆ? the result is +âˆ?
