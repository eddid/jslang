...
ejsval _ejs_function_new_utf8 (ejsval env, const char *name, EJSClosureFunc func)
{
...
ejsval
_ejs_function_new_utf8_without_proto (ejsval env, const char* name, EJSClosureFunc func)
{
    ejsval function_name = _ejs_string_new_utf8 (name);
    ejsval rv = _ejs_function_new_without_proto (env, function_name, func);
...
_ejs_Function_prototype_toString (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    char *utf8_funcname = _ejs_string_to_utf8(EJSVAL_TO_STRING(func_name));
...
_ejs_Function_prototype_apply (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    ejsval len = _ejs_specop_get(argArray, _ejs_atom_length, argArray);
    ...
    if (EJSVAL_IS_DENSE_ARRAY(argArray)) {
        ...
        while (index < n) {
            ...
            /*       argument. */
            ejsval nextArg = _ejs_specop_get(argArray, indexName, argArray);
...
bound_wrapper (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    if (bound_argc == 0) {
        ...
        ejsval* call_args = (ejsval*)malloc(sizeof(ejsval) * call_argc);
...
#define DEBUG_FUNCTION_ENTER(x) EJS_MACRO_START                         \
    if (trace) {                                                        \
        ...
        char *closure_utf8 = _ejs_string_to_utf8(EJSVAL_TO_STRING(closure_name)); \
        indent('*');                                                    \
        printf ("invoking %s\n", closure_utf8);                         \
        free (closure_utf8);                                            \
...
_ejs_function_specop_has_instance (ejsval F, ejsval V)
{
    ...
    ejsval O = _ejs_specop_get(F, _ejs_atom_prototype, F);
