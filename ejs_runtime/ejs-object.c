...
Put (ejsval O, ejsval P, ejsval V, EJSBool Throw)
{
    ...
    EJSBool success = _ejs_specop_set(O, P, V, O);
...
GetMethod (ejsval O, ejsval P)
{
    ...
    ejsval func = _ejs_specop_get(O, P, O);
...
{
#if 1
    //eddid: I generate hashcode for all of ejsval in JSValueHash, so no need to change key any more, just use it !
    return argument;
#else
...
uint32_t _ejs_object_hash (ejsval key)
{
    EJSObject* obj = EJSVAL_TO_OBJECT(key);
    if ((obj->hashcode & 1) == 0) {
        // just use the symbol's pointer
        obj->hashcode = ((uint32_t)(uintptr_t)obj) | 1;
    }
    return obj->hashcode;
}
...
{
    uint32_t result;
    switch (EJSVAL_TO_TAG(key))
    {
    case EJSVAL_TAG_STRING:
        result = _ejs_string_hash(key);
        break;
        
    case EJSVAL_TAG_OBJECT:
        result = _ejs_object_hash(key);
        break;
        
    case EJSVAL_TAG_MAGIC:
    case EJSVAL_TAG_INT32:
    case EJSVAL_TAG_BOOLEAN:
    case EJSVAL_TAG_NULL:
    case EJSVAL_TAG_UNDEFINED:
    default:
        result = hash6432shift(key.asInt64);
        break;
    }

    //printf("%s key.asInt64=0x%llx, hash=%d\n", __FUNCTION__, key.asInt64, result);
    return result;
}
...
FromPropertyDescriptor(EJSPropertyDesc* Desc)
{
    ...
    if (_ejs_property_desc_has_value(Desc)) {
        ...
        EJSPropertyDesc value_desc = { 0 };
		value_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		value_desc.value= Desc->value;
        _ejs_specop_define_own_property(obj, _ejs_atom_value, &value_desc, EJS_FALSE);
    }
    ...
    if (_ejs_property_desc_has_writable(Desc)) {
        ...
        EJSPropertyDesc writable_desc = { 0 };
		writable_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		writable_desc.value= BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_writable(Desc));
        _ejs_specop_define_own_property(obj, _ejs_atom_writable, &writable_desc, EJS_FALSE);
    }
    ...
    if (_ejs_property_desc_has_getter(Desc)) {
        ...
        EJSPropertyDesc get_desc = { 0 };
		get_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		get_desc.value= _ejs_property_desc_get_getter(Desc);
        _ejs_specop_define_own_property(obj, _ejs_atom_get, &get_desc, EJS_FALSE);
    }
    ...
    if (_ejs_property_desc_has_getter(Desc)) {
        ...
        EJSPropertyDesc set_desc = { 0 };
		set_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		set_desc.value= _ejs_property_desc_get_setter(Desc);
        _ejs_specop_define_own_property(obj, _ejs_atom_set, &set_desc, EJS_FALSE);
    }
    ...
    if (_ejs_property_desc_has_enumerable(Desc)) {
        ...
        EJSPropertyDesc enumerable_desc = { 0 };
		enumerable_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		enumerable_desc.value= BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_enumerable(Desc));
        _ejs_specop_define_own_property(obj, _ejs_atom_enumerable, &enumerable_desc, EJS_FALSE);
    }
    ...
    if (_ejs_property_desc_has_configurable(Desc)) {
        ...
        EJSPropertyDesc configurable_desc = { 0 };
		configurable_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
		configurable_desc.value= BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_configurable(Desc));
        _ejs_specop_define_own_property(obj, _ejs_atom_configurable, &configurable_desc, EJS_FALSE);
    }
...
ToPropertyDescriptor(ejsval Obj, EJSPropertyDesc *desc)
{
    ...
    if (_ejs_specop_has_property(Obj, _ejs_atom_enumerable)) {
        //    a. Let enum be Get(Obj, "enumerable"). 
        //    b. ReturnIfAbrupt(enum). 
        //    c. Set the [[Enumerable]] field of desc to ToBoolean(enum). 
        _ejs_property_desc_set_enumerable (desc, EJSVAL_TO_BOOLEAN(ToBoolean(_ejs_specop_get(Obj, _ejs_atom_enumerable, Obj))));
    }
    // 5. If HasProperty(Obj, "configurable") is true, then 
    if (_ejs_specop_has_property(Obj, _ejs_atom_configurable)) {
        //    a. Let conf be Get(Obj, "configurable"). 
        //    b. ReturnIfAbrupt(conf). 
        //    c. Set the [[Configurable]] field of desc to ToBoolean(conf). 
        _ejs_property_desc_set_configurable (desc, EJSVAL_TO_BOOLEAN(ToBoolean(_ejs_specop_get(Obj, _ejs_atom_configurable, Obj))));
    }
    // 6. If HasProperty(Obj, "value") is true, then 
    if (_ejs_specop_has_property(Obj, _ejs_atom_value)) {
        //    a. Let value be Get(Obj, "value"). 
        //    b. ReturnIfAbrupt(value). 
        //    c. Set the [[Value]] field of desc to value. 
        _ejs_property_desc_set_value (desc, _ejs_specop_get(Obj, _ejs_atom_value, Obj));
    }
    // 7. If HasProperty(Obj, "writable") is true, then 
    if (_ejs_specop_has_property(Obj, _ejs_atom_writable)) {
        //    a. Let writable be Get(Obj, "writable"). 
        //    b. ReturnIfAbrupt(writable). 
        //    c. Set the [[Writable]] field of desc to ToBoolean(writable). 
        _ejs_property_desc_set_writable (desc, EJSVAL_TO_BOOLEAN(ToBoolean(_ejs_specop_get(Obj, _ejs_atom_writable, Obj))));
    }
    // 8. If HasProperty(Obj, "get") is true, then 
    if (_ejs_specop_has_property(Obj, _ejs_atom_get)) {
        //    a. Let getter be Get(Obj, "get"). 
        //    b. ReturnIfAbrupt(getter). 
        ejsval getter = _ejs_specop_get(Obj, _ejs_atom_get, Obj);
    ...
    if (_ejs_specop_has_property(Obj, _ejs_atom_set)) {
        //    a. Let setter be Get(Obj, "set"). 
        //    b. ReturnIfAbrupt(setter). 
        ejsval setter = _ejs_specop_get(Obj, _ejs_atom_set, Obj);
...
_ejs_propertymap_rehash (EJSPropertyMap* map)
{
    ...
    for (_EJSPropertyMapEntry *s = map->head_insert; s; s = s->next_insert) {
        uint32_t hashcode = JSValueHash(s->name);
        int bucket = (int)(hashcode % map->nbuckets);
...
_ejs_propertymap_insert (EJSPropertyMap* map, ejsval name, EJSPropertyDesc* desc)
{
    ...
    _EJSPropertyMapEntry* new_s = (_EJSPropertyMapEntry*)malloc(sizeof(_EJSPropertyMapEntry));
    new_s->name = name;
    new_s->desc = desc;
...
/* property iterators */
typedef struct _EJSPropertyIterator {
    EJSObject obj;
...
EJS_DEFINE_CLASS(_EJSPropertyIterator,
...
{
    for (int i = 0; i < num; i ++) {
        if (!EJSChar_strcmp(EJSVAL_TO_FLAT_STRING(name), EJSVAL_TO_FLAT_STRING(keys[i])))
            return EJS_TRUE;
    }
    return EJS_FALSE;
}
...
_ejs_object_setprop (ejsval val, ejsval key, ejsval value)
{
    ...
    _ejs_specop_set(val, key, value, val);
...
_ejs_object_getprop (ejsval obj, ejsval key)
{
    if (EJSVAL_IS_NULL(obj) || EJSVAL_IS_UNDEFINED(obj)) {
        char* key_utf8 = _ejs_string_to_utf8(EJSVAL_TO_STRING(ToString(key)));
#if DEBUG_LAST_LOOKUP
        if (last_lookup) {
    ...
    if (EJSVAL_IS_PRIMITIVE(obj)) {
        if (EJSVAL_IS_STRING(obj)) {
            if (!EJSChar_strcmp(EJSVAL_TO_FLAT_STRING(ToString(key)), _ejs_buffer_length))
    ...
    last_lookup = EJSChar_strdup (EJSVAL_TO_FLAT_STRING(ToString(key)));
#endif

    return _ejs_specop_get(obj, key, obj);
}
...
{
    EJSPropertyDesc desc = { 0 };
	desc.flags = flags | EJS_PROP_FLAGS_VALUE_SET;
	desc.value = value;
    return _ejs_specop_define_own_property(obj, key, &desc, EJS_FALSE);
}

EJSBool _ejs_object_define_setter_property (ejsval obj, ejsval key, ejsval set, uint32_t flags)
{
    EJSPropertyDesc desc = { 0 };
	desc.flags = flags | EJS_PROP_FLAGS_SETTER_SET;
	desc.value = _ejs_undefined;
	desc.getter = _ejs_undefined;
	desc.setter = set;
    return _ejs_specop_define_own_property(obj, key, &desc, EJS_FALSE);
}
...
{
    EJSPropertyDesc desc = { 0 };
	desc.flags = flags | EJS_PROP_FLAGS_GETTER_SET;
	desc.value = _ejs_undefined;
	desc.getter = get;
	desc.setter = _ejs_undefined;
    return _ejs_specop_define_own_property(obj, key, &desc, EJS_FALSE);
}
...
_ejs_dump_value (ejsval val)
{
    if (EJSVAL_IS_UNDEFINED(val)) {
        ...
        char* val_utf8 = _ejs_string_to_utf8(EJSVAL_TO_STRING(val));
        _ejs_log ("string: '%s'\n", val_utf8);
        free (val_utf8);
    }
    else if (EJSVAL_IS_OBJECT(val)) {
        _ejs_log ("<object %s>\n", OP_class_name(EJSVAL_TO_OBJECT(val)));
    }
}
...
_ejs_Object_getPrototypeOf (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    return _ejs_specop_get_prototype_of(obj);
}
...
_ejs_Object_setPrototypeOf (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    EJSBool status = _ejs_specop_set_prototype_of(O, proto);
...
_ejs_Object_getOwnPropertyDescriptor (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    EJSPropertyDesc* desc = _ejs_specop_get_own_property(obj, key, NULL);
    ...
    for (int source_i = 1; source_i < argc; source_i ++) {
        ...
        for (_EJSPropertyMapEntry* s = from_->map.head_insert; s; s = s->next_insert) {
            ...
            if (desc && _ejs_property_desc_is_enumerable(desc)) {
                ...
                ejsval propValue = _ejs_specop_get(from, nextKey, from);
...
_ejs_Object_defineProperty (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    if (!EJSVAL_IS_OBJECT(O)) {
        ...
        char* utf8_name = _ejs_string_to_utf8(EJSVAL_TO_STRING(name));
    ...
    _ejs_specop_define_own_property(O, name, &desc, EJS_TRUE);
...
_ejs_Object_defineProperties (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    ejsval* names = (ejsval*)malloc(names_len * sizeof(ejsval));
    int n = 0;
    for (_EJSPropertyMapEntry *s = props_obj->map.head_insert; s; s = s->next_insert) {
        if (_ejs_property_desc_is_enumerable(s->desc))
            names[n++] = s->name;
    ...
    DefinePropertiesPair *descriptors = (DefinePropertiesPair *)malloc(sizeof(DefinePropertiesPair) * names_len);
    ...
    for (int n = 0; n < names_len; n ++) {
        ...
        ejsval descObj = _ejs_specop_get(props, P, props);
    ...
    for (int d = 0; d < names_len; d++) {
        ...
        _ejs_specop_define_own_property(O, P, desc, EJS_TRUE);
    }
...
DefinePropertyOrThrow (ejsval O, ejsval P, EJSPropertyDesc* desc, ejsval *exc)
{
    ...
    EJSBool success = _ejs_specop_define_own_property(O, P, desc, EJS_FALSE);
...
SetIntegrityLevel (ejsval O, IntegrityLevel level)
{
    ...
    ejsval keysArray = _ejs_specop_own_property_keys(O);
    ...
    if (n > 0) {
        keys = (ejsval *)malloc(sizeof(ejsval) * n);
        for (int i = 0; i < n; i ++) {
    ...
    // 8. Else level is "frozen", 
        ...
        for (int i = 0; i < n; i ++) {
            ...
            ejsval exc;
            EJSPropertyDesc* currentDesc = _ejs_specop_get_own_property(O, k, &exc);
            //       ii. If status is an abrupt completion, then 
    ...
    return BOOLEAN_TO_EJSVAL(_ejs_specop_preventextensions(O));
}
...
_ejs_Object_preventExtensions (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 3. ReturnIfAbrupt(status).
    EJSBool status = _ejs_specop_preventextensions(O);
...
_ejs_Object_isExtensible (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    return BOOLEAN_TO_EJSVAL(_ejs_specop_is_extensible(O));
}
...
_ejs_Object_prototype_toString (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    EJSBool hasTag = _ejs_specop_has_property(O, _ejs_Symbol_toStringTag);
    ...
    // 18. Else, 
        ...
        tag = _ejs_specop_get(O, _ejs_Symbol_toStringTag, O);
...
_ejs_Object_prototype_toLocaleString (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    ejsval toString = _ejs_specop_get(O, _ejs_atom_toString, O);
...
_ejs_Object_prototype_hasOwnProperty (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    return BOOLEAN_TO_EJSVAL(_ejs_specop_get_own_property(_this, needle, NULL) != NULL);
}
...
_ejs_Object_prototype_isPrototypeOf (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    while (EJS_TRUE) {
        ...
        V = _ejs_specop_get_prototype_of(V);
...
_ejs_Object_prototype_propertyIsEnumerable (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    EJSPropertyDesc* desc = _ejs_specop_get_own_property(O, P, NULL);
...
_ejs_object_specop_set_prototype_of (ejsval O, ejsval V)
{
    ...
    if (!EJSVAL_IS_NULL(V)) {
        ...
        while (!EJSVAL_IS_NULL(p)) {
            ...
            ejsval nextp = _ejs_specop_get_prototype_of(p);
...
_ejs_object_specop_get (ejsval O, ejsval P, ejsval Receiver)
{
    ...
    if (EJSVAL_IS_STRING(pname) && !EJSChar_strcmp(_ejs_buffer___proto__, EJSVAL_TO_FLAT_STRING(pname)))
        return _ejs_specop_get_prototype_of(O);
    ...
    if (desc == NULL) {
        ...
        ejsval parent = _ejs_specop_get_prototype_of(O);
        ...
        return _ejs_specop_get(parent, P, Receiver);
    }
...
{
    EJSPropertyDesc undefined_desc = { 0 };
	undefined_desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE;
	undefined_desc.value = _ejs_undefined;

    // 1. Assert: IsPropertyKey(P) is true. 
    P = ToPropertyKey(P); // XXX this shouldn't be necessary, but js passes numbers here
    
    // 2. Let ownDesc be the result of calling the [[GetOwnProperty]] internal method of O with argument P. 
    // 3. ReturnIfAbrupt(ownDesc). 
    EJSPropertyDesc* ownDesc = _ejs_specop_get_own_property(O, P, NULL);
    ...
    if (!ownDesc) {
        ...
        if (!EJSVAL_IS_NULL(parent)) {
            ...
            return _ejs_specop_set(parent, P, V, Receiver);
        }
        //    d. Else, 
        else {
    ...
    if (IsDataDescriptor(ownDesc)) {
        ...
        EJSPropertyDesc* existingDescriptor = _ejs_specop_get_own_property(Receiver, P, NULL);
        ...
        if (existingDescriptor) {
            ...
            EJSPropertyDesc valueDesc = { 0 };
			valueDesc.flags = EJS_PROP_FLAGS_VALUE_SET;
			valueDesc.value = V;
            return _ejs_specop_define_own_property(Receiver, P, &valueDesc, EJS_FALSE); /* XXX not sure about last parameter here, it's an ES5-ism */
        }
...
_ejs_object_specop_has_property (ejsval O, ejsval P)
{
    ...
    EJSPropertyDesc* hasOwn = _ejs_specop_get_own_property(O, P, NULL);
    ...
    if (!EJSVAL_IS_NULL(parent)) {
        ...
        return _ejs_specop_has_property(parent, P);
    }
    // 8. Return false. 
    return EJS_FALSE;
}
...
#define REJECT(reason)                                                  \
    ...
    EJSPropertyDesc* current = _ejs_specop_get_own_property(O, P, NULL);
...
_ejs_object_specop_own_property_keys (ejsval O)
{
    ...
    ejsval* numberkeys = (ejsval *)malloc(sizeof(ejsval) *O_->map.inuse);
    int num_numberkeys = 0;
    ejsval* stringkeys = (ejsval *)malloc(sizeof(ejsval) *O_->map.inuse);
    int num_stringkeys = 0;
    ejsval* symbolkeys = (ejsval *)malloc(sizeof(ejsval) *O_->map.inuse);
