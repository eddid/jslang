...
_ejs_Set_prototype_add (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    // 7. If value is �?, then let value be +0. 
    ...
    e = (EJSSetValueEntry *)calloc (1, sizeof (EJSSetValueEntry));
    e->value = value;
...
#define PROTO_GETTER(x) EJS_INSTALL_ATOM_GETTER(_ejs_Set_prototype, x, _ejs_Set_prototype_get_##x)
    ...
    ejsval _values = _ejs_function_new_without_proto (_ejs_null, _ejs_atom_values, (EJSClosureFunc)_ejs_Set_prototype_values);
