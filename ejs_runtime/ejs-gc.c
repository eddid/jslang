...
#define PTR_TO_ARENA_PAGE_BASE(ptr) ((void*)ALIGN((uintptr_t)PTR_TO_ARENA(ptr) + sizeof(Arena), PAGE_SIZE))
...
#define PTR_TO_CELL(ptr,info) (((uintptr_t)(ptr) - (uintptr_t)(info)->page_start) / (info)->cell_size)
...
static int finalizer_cond;
static int big_gc_lock; // actually recursive, see _ejs_gc_init
static int arena_lock;

#define LOCK_GC() do {                                                  \
        /*_ejs_log ("locking gc at %s:%d\n", __PRETTY_FUNCTION__, __LINE__); */ \
        jsextern_mutex_lock(big_gc_lock);                               \
    } while (0)
#define UNLOCK_GC() do {                        \
        /*_ejs_log ("unlocking gc at %s:%d\n", __PRETTY_FUNCTION__, __LINE__); */ \
        jsextern_mutex_unlock(big_gc_lock);     \
    } while (0)

#define LOCK_ARENAS() do {                                                  \
        /*_ejs_log ("locking arenas at %s:%d\n", __PRETTY_FUNCTION__, __LINE__); */ \
        jsextern_mutex_lock(arena_lock);                               \
    } while (0)
#define UNLOCK_ARENAS() do {                        \
        /*_ejs_log ("unlocking arenas at %s:%d\n", __PRETTY_FUNCTION__, __LINE__); */ \
        jsextern_mutex_unlock(arena_lock);     \
    } while (0)
static int finalizer_thread;
...
#define SET_GRAY(cell) EJS_MACRO_START                                  \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, (_bc & ~CELL_COLOR_MASK) | CELL_GRAY_MASK)); \
    EJS_MACRO_END
...
#define SET_WHITE(cell) EJS_MACRO_START                                 \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, (_bc & ~CELL_COLOR_MASK) | white_mask)); \
    EJS_MACRO_END
...
#define SET_BLACK(cell) EJS_MACRO_START                                 \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, (_bc & ~CELL_COLOR_MASK) | black_mask)); \
    EJS_MACRO_END
...
#define SET_FREE(cell) EJS_MACRO_START                                  \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, CELL_FREE));    \
    EJS_MACRO_END
...
#define SET_FINALIZABLE(cell) EJS_MACRO_START                           \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, CELL_FINALIZABLE)); \
    EJS_MACRO_END
...
#define SET_ALLOCATED(cell) EJS_MACRO_START                             \
    ...
    do {                                                                \
        _bc = (cell);                                                   \
    } while (!jsextern_os_swap ((__int64*)&cell, _bc, (_bc & ~CELL_FREE))); \
    EJS_MACRO_END
...
arena_new()
{
    ...
    Arena* new_arena = (Arena *)arena_start;

    memset (new_arena, 0, sizeof(Arena));

    new_arena->end = (char *)arena_start + ARENA_SIZE;
    new_arena->pos = (void*)ALIGN((char *)arena_start + sizeof(Arena), PAGE_SIZE);
...
alloc_page_info_from_arena(Arena *arena, void *page_data, size_t cell_size)
{
    ...
    info->page_end = (char *)info->page_start + PAGE_SIZE;
    ...
    info->page_mutex = jsextern_mutex_create();
    return info;
}
...
alloc_page_from_arena(Arena *arena, size_t cell_size)
{
    ...
    if (arena->free_pages) {
        ...
        info->bump_ptr = info->page_start;
        memset (info->page_bitmap, CELL_FREE, info->num_cells * sizeof(BitmapCell));
        SPEW(3, _ejs_log ("alloc_page_from_arena from free pages for cell size %d = %p\n", info->cell_size, info));
        return info;
    }
        ...
        arena->pos = (char *)page_data + PAGE_SIZE;
        arena->pages[page_idx] = page_data;
        arena->page_infos[page_idx] = info;
        SPEW(3, _ejs_log ("alloc_page_from_arena from bump pointer for cell size %d = %p\n", info->cell_size, info));
        return info;
    }
    else {
        return NULL;
...
{
    SPEW(2, _ejs_log ("allocating new page for cell size %d\n", cell_size));
    PageInfo *rv = NULL;
    for (int i = 0; i < num_arenas; i ++) {
...
finalize_object(GCObjectPtr p)
{
    ...
    if ((*headerp & EJS_SCAN_TYPE_OBJECT) != 0) {
        ...
        _ejs_specop_finalize((EJSObject *)p);
    }
...
{
    while (EJS_TRUE) {
        jsextern_os_msleep (1);

        EJSFinalizerEntry* this_run;

        do {
            this_run = finalizer_list;
        } while (!jsextern_os_swap ((__int64*)&finalizer_list, (__int64)this_run, NULL));
        ...
        while (this_run) {
            ...
            SPEW(3, _ejs_log ("finalized object %p in page %p, num_free_cells == %d\n", fin->gcobj, info, info->num_free_cells + 1));
            // if this page is empty, move it to this arena's free list
            jsextern_mutex_lock (info->page_mutex);
            info->num_free_cells ++;
            jsextern_mutex_unlock (info->page_mutex);

            if (info->num_free_cells == info->num_cells) {
                LOCK_GC();
                if (info->num_free_cells == info->num_cells) {
                    if (info->los_info) {
                        _ejs_log ("releasing large object (size %d)!\n", info->los_info->alloc_size);
        ...
        jsextern_signal_send(finalizer_cond);
    }
...
_ejs_gc_init()
{
    finalizer_cond = jsextern_signal_create();
    big_gc_lock = jsextern_mutex_create();
    arena_lock = jsextern_mutex_create();
    ...
    finalizer_thread = jsextern_thread_create ((void *(*) (void *))_ejs_finalizer_thread, NULL);
}
...
{
    _ejs_specop_scan(obj, _scan_ejsvalue);
}
...
#endif
    for (; p < (char *)high - sizeof(ejsval); p += sizeof(ejsval)) {
            ...
            if (info->num_free_cells == info->num_cells) {
                ...
                for (int c = 0, ce = info->num_cells; c < ce; c ++) {
                    ...
                    if (!IS_FINALIZABLE(cell) && IS_WHITE(cell)) {
                        ...
                        fin->gcobj = (char*)info->page_start + c * info->cell_size;
                        ...
                        } while (!jsextern_os_swap ((__int64*)&finalizer_list, (__int64)fin->next, (__int64)fin));
...
#define MARK_REGISTERS EJS_MACRO_START \
    ...
    EJS_MACRO_END
#elif TARGET_CPU_X86 || defined(WIN32)
...
mark_thread_stack()
{
    ...
    //eddid: how to set stack_bottom ?
    //mark_ejsvals_in_range(((char*)&stack_top) + sizeof(GCObjectPtr), stack_bottom);
}
...
alloc_from_page(PageInfo *info)
{
    ...
    SPEW(2, _ejs_log ("allocating object from page %p (cell size %d)\n", info, info->cell_size));

    if (info->bump_ptr) {
        rv = (GCObjectPtr)ALIGN((char*)info->bump_ptr, 8);
        cell = PTR_TO_CELL(info->bump_ptr, info);
        info->bump_ptr = (char*)info->bump_ptr + info->cell_size;
        // check if we can service the next alloc request from the bump_ptr.  if we can't, switch
        // to the freelist code below.
        //eddid change >= to >, the last block should be used
        if ((char*)info->bump_ptr + info->cell_size > (char*)info->page_end)
            info->bump_ptr = NULL;
    }
    else {
        for (cell = 0; cell < info->num_cells; cell ++) {
            if (IS_FREE(info->page_bitmap[cell])) {
                rv = (char*)info->page_start + (cell * info->cell_size);
                break;
            }
        }
    }
    ...
    SPEW(2, _ejs_log ("allocated obj %p from page %p (cell size %d), free cells remaining %d\n", rv, info, info->cell_size, info->num_free_cells));
...
alloc_from_los(size_t size, EJSScanType scan_type)
{
    ...
    LargeObjectInfo *rv = (LargeObjectInfo *)alloc_from_os(size + sizeof(LargeObjectInfo) + 16, 0);
    if (rv == NULL)
        return NULL;

    rv->page_info.page_bitmap = (char*)((char*)rv + sizeof(LargeObjectInfo)); // our bitmap comes right after the header
    rv->page_info.page_start = (void*)ALIGN((char*)rv + sizeof(LargeObjectInfo) + 8, 8);
...
_ejs_gc_alloc(size_t size, EJSScanType scan_type)
{
    ...
    if (bucket > OBJECT_SIZE_HIGH_LIMIT_BITS) {
        SPEW(2, _ejs_log ("need to alloc %d from los!!!\n", size));
        rv = alloc_from_los(size, scan_type);
        if (rv == NULL) {
            if (num_allocs == 0) {
                _ejs_log ("los allocation (size = %d) failed twice, throwing\n", size);
                _ejs_throw (los_allocation_failed_exc);
            }
            else {
                _ejs_log ("los allocation (size = %d) failed, trying to collect\n", size);
                UNLOCK_GC();
                _ejs_gc_collect();
                jsextern_signal_wait(finalizer_cond);
                alloc_size_at_last_gc = alloc_size;
                num_allocs = 0;
...
_ejs_GC_dumpAllocationStats (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    if (argc > 0) {
        tag = _ejs_string_to_utf8(EJSVAL_TO_STRING(args[0]));
    }
