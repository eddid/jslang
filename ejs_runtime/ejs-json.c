...
_ejs_JSON_parse (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    /* 2. Parse JText using the grammars in 15.12.1. Throw a SyntaxError exception if JText did not conform to the 
       EJSON grammar for the goal symbol EJSONText.  */
    char *flattened_jtext =  _ejs_string_to_utf8(EJSVAL_TO_STRING(jtext));

    /* 3. Let unfiltered be the result of parsing and evaluating JText as if it was the source text of an ECMAScript 
       Program but using EJSONString in place of StringLiteral. Note that since JText conforms to the EJSON 
...
Quote(StringifyState *state, ejsval value)
{
    ...
    jschar *product = (jschar *)malloc((len * 4 + 2) * sizeof(jschar));
    ...
    product[pi++] = '\"';
    /* 4. Return product. */
    ejsval rv = _ejs_string_new_len(product, pi);
    free (product);
    return rv;
}
...
Str(StringifyState *state, ejsval key, ejsval holder)
{
    ...
    ejsval value = _ejs_specop_get(holder, key, holder);
    ...
    if (EJSVAL_IS_OBJECT(value)) {
        ...
        ejsval toJSON = _ejs_specop_get(holder, _ejs_atom_toJSON, holder);
...
_ejs_JSON_stringify (ejsval env, ejsval _this, uint32_t argc, ejsval *args)
{
    ...
    /* 7. Else if Type(space) is String */
        ...
        state.gap = _ejs_string_new_len (flattened, 10);
    }
    /* 8. Else */
    else {
	...
	desc.value = value;
	desc.flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE;
    _ejs_specop_define_own_property(wrapper, _ejs_atom_empty, &desc, EJS_FALSE);
